 ...
 *
 * If the OpenAPI definition includes "query" parameters, they can be used to filter the results.
 * Deep property names are allowed (e.g. "?address.city=New+York").
 * Query string params that are not defined in the OpenAPI definition are ignored.
 *
 * @param   {Request}   req
 * @param   {Response}  res
 * @param   {function}  next
  ...
  dataStore.getCollection(req.path, (err, resources) => {
    if (!err) {
      ...
      if (resources.length === 0) {
        ...
        res.openapi.lastModified = new Date();

        // Use the default/example value, if there is one
        if (res.openapi.schema) {
          resources = res.openapi.schema.default || res.openapi.schema.example || [];
        }
      }
      else {
        // Determine the max "modifiedOn" date of the remaining items
        res.openapi.lastModified = _.maxBy(resources, "modifiedOn").modifiedOn;
 ...
 *
 * If the OpenAPI definition includes "query" parameters, they can be used to filter what gets deleted.
 * Deep property names are allowed (e.g. "?address.city=New+York").
 * Query string params that are not defined in the OpenAPI definition are ignored.
 *
 * @param   {Request}   req
  ...
  dataStore.getCollection(req.path, (err, resources) => {
    if (err) {
      ...
      if (resourcesToDelete.length === 0) {
        ...
        dataStore.deleteCollection(req.path, sendResponse, req);
      }
      else {
...
/**
 * Filters the given {@link Resource} array, using the "query" params defined in the OpenAPI definition.
 *
...
function filter (resources, req) {
  ...
  if (resources.length > 0) {
    ...
    let queryParams = _.filter(req.openapi.params, { in: "query" });
