...
/**
 * Describes the semantics of an OpenAPI response.
 *
 * @param   {object}    response - The Response object, from the OpenAPI definition
 * @param   {object}    path     - The Path object that contains the response. Used for semantic analysis.
 * @param   {object}    accepts  - The accepts method from the request, to check for accepted response types
 * @constructor
 */
function SemanticResponse (response, path, accepts) {
  const contentType = Object.keys(response.content).filter(type => accepts(type))[0];
  
  /**
   * The JSON schema of the response
   * @type {object|null}
   */
  this.schema = contentType ? response.content[contentType].schema || null : null;

  /**
   * The response headers, from the OpenAPI definition
   * @type {object|null}
   */
   ...
   * This is used to set the Last-Modified HTTP header (if defined in the OpenAPI definition)
   *
   ...
   * This is used to set the Location HTTP header (if defined in the OpenAPI definition)
   *
  ...
  if (!this.isEmpty) {
    this.setWrapperInfo(response.content[contentType], path);
  }
}
